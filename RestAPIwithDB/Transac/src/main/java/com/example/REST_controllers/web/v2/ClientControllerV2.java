...
import com.example.REST_controllers.web.model.CreateClientWithOrderRequest;
...
public class ClientControllerV2
{
    ...
    public ResponseEntity<ClientResponse> createWithOrders(@RequestBody CreateClientWithOrderRequest request)
    {
        ...
        List<Order> orders = request.getOrders().stream().map(orderRequest -> Order.builder()
                ...
                .build()).toList();
        return ResponseEntity.status(HttpStatus.CREATED).body(clientMapperV2.clientToResponse(dataBaseClientService.saveWithOrders(client,orders)));
    }
}
